...
* se aprovada, o valor da transação é debitado do balance disponível do benefício, e informamos à maquininha que tudo ocorreu bem. 
 ...
 * a criação de cartões (todo cartão deverá ser criado com um balance inicial de R$500,00)
 * a obtenção de balance do cartão
...
Uma transação pode ser autorizada se:
   ...
   * o cartão possuir balance disponível
...
Não é necessário persistir a transação. Mas é necessário persistir o cartão criado e alterar o balance do cartão caso uma transação ser autorizada pelo sistema.
 ...
 * verificação do balance do cartão recém-criado
 * realização de diversas transações, verificando-se o balance em seguida, até que o sistema retorne informação de balance insuficiente
...
Exemplo: dado que um cartão possua R$10.00 de balance. Se fizermos 2 transações de R$10.00 ao mesmo tempo, em instâncias diferentes da aplicação, como o sistema deverá se comportar?